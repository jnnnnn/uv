...
impl SourceBuild {
    ...
    pub async fn get_metadata_without_build(&mut self) -> Result<Option<PathBuf>, Error> {
        ...
        let script = formatdoc! {
            ...
            if prepare_metadata_for_build_wheel:
                ...
                print()
            "#, pep517_backend.backend_import(), escape_path_for_python(&metadata_directory), self.config_settings.escape_for_python()
        };
        let span = info_span!(
            "run_python_script",
    ...
    async fn pep517_build(
        ...
        let script = formatdoc! {
            ...
            "#, pep517_backend.backend_import(), self.build_kind, escaped_wheel_dir,  metadata_directory, self.config_settings.escape_for_python()
        };
        let span = info_span!(
            "run_python_script",
