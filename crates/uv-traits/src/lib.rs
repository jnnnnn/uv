...
impl ConfigSettings {
    ...
    pub fn escape_for_python(&self) -> String {
        ...
        for (key, value) in &self.0 {
            ...
            result.push('"');
            result.push_str(&key.escape_default().to_string());
            result.push('"');
            result.push_str(": ");
            match value {
                ConfigSettingValue::String(value) => {
                    result.push('"');
                    result.push_str(&value.escape_default().to_string());
                    result.push('"');
                }
...
mod tests {
    ...
    fn escape_for_python() {
        ...
        settings.0.insert(
            "key".to_string(),
            ConfigSettingValue::String("val{}ue".to_string()),
        );
        assert_eq!(settings.escape_for_python(), r#"{"key": "val{}ue"}"#);
    }
}
